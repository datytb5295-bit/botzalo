...
export let groupSettings = readGroupSettings(); // KHAI BÁO BIẾN groupSettings Ở ĐÂY
...
api.listener.on("message", async (message) => {
  try {
    const threadId = message.threadId; // LẤY threadId TỪ SỰ KIỆN TIN NHẮN
    const nameGroup = await getGroupName(api, threadId); // LẤY TÊN NHÓM
    
    // GỌI HÀM messagesUser VỚI ĐẦY ĐỦ THAM SỐ
    await messagesUser(api, message, threadId, groupSettings, nameGroup, false);
